...
def ask_rag(question: str, stream: bool = False):
    """
    ...
    # Build prompt (concise for faster responses)
    if context:
...
Answer:"""
    ...
    if stream:
        ...
        return ask_ollama(prompt, stream=True)
    else:
        # Non-streaming call - ask_ollama is synchronous
        try:
            return ask_ollama(prompt, stream=False)
        except Exception as e:
