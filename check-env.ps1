...
Write-Host "🔍 RAG Chatbot Environment Check" -ForegroundColor Cyan
...
try {
    ...
    if ($pythonVersion -match "Python 3\.(1`[1-9`]|`[2-9`]\d)") {
        ...
        Write-Host "⚠️  $pythonVersion (Python 3.11+ recommended)" -ForegroundColor Yellow
        $allGood = $false
    }
} catch {
...
if (-not $ollamaInstalled) {
    ...
    try {
        ...
        Write-Host "❌ Ollama service not running. Run: ollama serve" -ForegroundColor Red
        $allGood = $false
    }
}
...
if ($allGood) {
    Write-Host "🎉 All checks passed! You're ready to run the chatbot." -ForegroundColor Green
    Write-Host ""
    Write-Host "Next steps:" -ForegroundColor Cyan
    Write-Host "1. Run: .\start.ps1" -ForegroundColor White
    Write-Host "2. Open: http://localhost:3000" -ForegroundColor White
} else {
    Write-Host "⚠️  Some issues need to be fixed before running." -ForegroundColor Yellow
